...
(function($, undefined) {
  ...
  $.extend(Datepicker.prototype, {
	...
	_showDatepicker: function(input) {
		...
		var rightOffset = (inst.dpDiv.innerWidth() - $(input).width()) / 2;
		$.datepicker._updateDatepicker(inst);
		inst.dpDiv.css({
			"position": ($.datepicker._inDialog && $.blockUI ? "static" : (isFixed ? "fixed" : "absolute")),
			"display": "none",
			"left": offset.left - rightOffset + "px",
			"top": offset.top + "px"
		});
		if (!inst.inline) {
	...
	_generateHTML: function(inst) {
	  ...
	  for (var row = 0; row < numMonths[0]; row++) {
		...
		for (var col = 0; col < numMonths[1]; col++) {
		  ...
		  console.log("Log-DaysInMonth:"+daysInMonth)
		  ...
		  console.log("Log_inst.selectedDay"+inst.selectedDay)
...
function leap_persian(year) {
	isLeap = false;
	if ( year == 1399 || year == 1403 || year == 1408 ) { 
		isLeap = true;
	}
	return isLeap;
  }
...
jQuery(function(a) {
  a.datepicker.regional.fa = {
	...
	nextText: "بعد",
	currentText: "امروز",
	monthNames: ["فروردین", "اردیبهشت", "خرداد", "تیر", "اَمرداد", "شهریور", "مهر", "آبان", "آذر", "دی", "بهمن", "اسفند"],
	monthNamesShort: ["فروردین", "اردیبهشت", "خرداد", "تیر", "اَمرداد", "شهریور", "مهر", "آبان", "آذر", "دی", "بهمن", "اسفند"],
